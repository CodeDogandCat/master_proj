...
public class DownLoadApk {
        ...
        if (downloadId != -1L) {
            ...
            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                ...
                if (status == DownloadManager.STATUS_FAILED) {
                    ...
                    sp.edit()
                            .remove(DownloadManager.EXTRA_DOWNLOAD_ID)
                            .apply();
                }
    ...
    private static void start(Context context, String url, String title, String appName) {
        long id = FileDownloadManager.getInstance(context)
                .startDownload(url,
                        title, "下载完成后点击打开", appName);
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        sp.edit()
                .putLong(DownloadManager.EXTRA_DOWNLOAD_ID, id)
                .commit();
